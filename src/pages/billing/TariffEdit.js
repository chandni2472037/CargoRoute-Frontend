...
import { getTariffById, updateTariff } from '../../api/billingApi';
...
export default function TariffEdit() {
    ...
    getTariffById(id)
      .then((tariff) => {
        setForm({
          ...
          ratePerKg:     tariff.ratePerKg     != null ? String(tariff.ratePerKg)  : '',
          ratePerM3:     tariff.ratePerM3     != null ? String(tariff.ratePerM3)  : '',
          minCharge:     tariff.minCharge     != null ? String(tariff.minCharge)  : '',
          effectiveFrom: tariff.effectiveFrom ? tariff.effectiveFrom.substring(0, 10) : '',
          effectiveTo:   tariff.effectiveTo   ? tariff.effectiveTo.substring(0, 10)   : '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTimeout(() => navigate('/billing/tariffs'), 1200);
    } catch (e) {
      setError(e.message);
    } finally {
      setSaving(false);
    ...
    <Layout>
      <div style={{ maxWidth: '100%', width: '100%', margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
          ...
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1a2b45', margin: 0 }}>✏️ Edit Tariff</h1>
            <p style={{ fontSize: 13, color: '#94a3b8', margin: '4px 0 0' }}>Update the tariff details below.</p>
          </div>
        </div>
